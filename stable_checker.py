...
class IntegerWrapper(int):
    ...
    def is_sequence_stable(seq: Sequence[IntegerWrapper]):
        return all(seq[i].key_compare(seq[i + 1]) <= 0 for i in range(len(seq) - 1))
    
    @staticmethod
    def sequence_out_of_place(seq: Sequence[IntegerWrapper]):
        for i in range(len(seq) - 1):
            if seq[i].key_compare(seq[i + 1]) == 1:
                return i
        return -1
...
def main(colonfunc: str):
    ...
    stable = IntegerWrapper.sequence_out_of_place(result)
    if stable == -1:
        print(colonfunc, 'is stable')
    else:
        print(colonfunc, 'is not stable (indices', stable, 'and', stable + 1, 'are out of order)')
    print('   ', result[:10], '...')
